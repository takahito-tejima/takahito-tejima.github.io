    ...
    function(vertsIndices, patchIndex, type, quadOffset)
{
    ...
    var rp;
    var valenceTable = mesh.valenceTable;
    var quadOffsets = mesh.quadOffsets;
    var verts = mesh.patchVerts;
    var zerothNeighbors = [];
    var org = this.org;
    var opos = this.opos;
    var e0 = this.e0;
    ...
    function(vertsIndices, patchIndex, u, v)
{
    for (var i = 0; i < 20; ++i) {
        ...
        vec3.set(this.GP[i], mesh.patchVerts[vid*3], mesh.patchVerts[vid*3+1], mesh.patchVerts[vid*3+2]);
    }
    return this.evalGregoryUV(this.GP, u, v);
}
    ...
    function(vertsIndices, patchIndex, u, v)
{
    ...
    for (var i = 0; i < ncp; ++i) {
        var x = mesh.patchVerts[vertsIndices[patchIndex*16+i]*3+0];
        var y = mesh.patchVerts[vertsIndices[patchIndex*16+i]*3+1];
        var z = mesh.patchVerts[vertsIndices[patchIndex*16+i]*3+2];
